...
def jira_url_search = "https://arbisoft123.atlassian.net/rest/api/2/search/?jql=project%3DDEVOPS%20AND%20summary%20~%20%27security%20vulnerbility%20found%27%20AND%20status%20not%20in%20(done%2C%20resolved%2C%20Canceled)&fields=id"
      


//def proc = [ "curl", "-u", "${authString}", "-X", "POST" ,"--data", "${body_req}", "-H", "Content-Type: application/json", "${jira_url}"].execute()
def proc = [ "curl", "-u", "${authString}", "-X", "GET", "-H", "Content-Type: application/json", "${jira_url_search}"].execute()
def stuff = new JsonSlurper().parseText(proc.toString())
println(proc.text)
